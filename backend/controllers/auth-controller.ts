...
const login = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return next(createError(500, error.message, "ER_UNEXP"));
  }
  finally {
    connection.release();
  }
...
const signup = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (emailExists.length != 0) {
      return next(createError(409, "User with the provided email already exists!", "ER_EMAIL_EXISTS"));
    }
    ...
    if (phoneNumberExists.length != 0) {
      return next(createError(409, "User with the provided phone number already exists!", "ER_PN_EXISTS"));
    }
